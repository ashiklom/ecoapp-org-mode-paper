...
The current PFTs are products of expert knowledge from an era when computational power was much more limited and data on functional diversity were far fewer (CITE: Prentice 199X, etc.); this study, among others, points to their limitations.
The current PFT structure is, however, not immutable.
One alternative would be to explicitly account for systematic differences in trait values between regions with similar climates (e.g. Butler 2017).
A second alternative is to further disaggregate PFTs based on successional stage, shade tolerance, or [something…] (Longo ED2 paper; LPJ-GUESS?).
A third is to allow PFTs to emerge from the data through classification and clustering techniques applied to functional traits and other ecological characteristics cite:boulangeat_2012_improving.
Finally, one could eschew PFTs altogether in favor of modeling individual species cite:post_1996_linkages,weng_2015_scaling, or abandon discrete categories altogether and model the continuous trait space (e.g. Scheiter, etc.).
