...
Within PFTs, for all traits except leaf lifespan, slopes between trait pairs were usually consistent in direction, though with a few exceptions (Fig.\ 1).
Mass- and area-normalized traits (N, P, R_d, V_{c,max}, J_{max}) usually had positive relationships with each other and, respectively, positive and negative relationships with SLA.
A notable exception was broadleaved deciduous temperate trees (BlDTe),
which had positive slopes for SLA-R_{d,area} and SLA-J_{max,area} (negative for most other PFTs)
and a negative slope for N-R_d (positive for most other PFTs).
Mass-based traits and SLA usually had negative relationships with leaf lifespan,
but more of these had credible intervals that intersected zero, and the mean direction of the slopes was different for a few PFTs:
Broadleaved deciduous temperate (BlDTe) and boreal trees (BlDBo), and boreal deciduous shrubs (ShDBo), had positive SLA-leaf lifespan relationships,
and C3 arctic grasses (C3GAr) and C4 grasses had positive relationships of leaf lifespan with SLA, N_mass, P_mass, and R_{d,mass}.
Slopes between leaf lifespan and area-normalized traits were even more variable, both by PFT and trait.
For example, among broadleaved trees, the slope of the leaf lifespan-N_area relationship was positive for tropical species (BlETr and BlDTr) but negative for temperate (BlETe, BlDTe) and boreal (BlDTe) species.

In terms of mass vs. area normalization, across-PFT relationships for mass-normalized traits were stronger on average.
All mass-normalized traits and SLA had a relatively strong negative across-PFT relationship with leaf lifespan
and generally positive relationships with each other, with the exception of J_{max,mass}.
Across-PFT relationships among area-normalized traits were less consistent, with relatively strong relationships only for leaf lifespan with SLA (negative), N_area (positive), and P_area (positive).
However, an important caveat to these results is that all of the across-PFT slopes had credible intervals that intersected zero.
This is due to the relatively small number of points (14) used to estimate the across-PFT covariance matrix.
...
Moreover, because all across-PFT slopes have the same sample size (unlike PFT-level estimates, where sample size varies dramatically by PFT and trait pair),
we can reliably draw inference from the relative strength of pairwise across-PFT trait relationships.

The impact of sample size on ability to confidently estimate pairwise trait relationships is evident in within PFTs as well.
Many of the PFTs for which a large number of pairwise trait slopes are near zero--most notably, all of the high-latitude PFTs (BlDBo, NlEBo, NlD, ShDBo, and C3GAr)--all have relatively small sample sizes (Tab.\ 1).
Importantly, for estimating covariance, pairwise observations of multiple traits on the same individual are required, but the number of pairwise observations for many of these traits was often zero (Tab. S4).
...
As a method for imputing trait observations, our hierarchical model consistently outperformed the univariate approach (Fig. 3).
In every case, the RMSE of the hierarchical model was lower than that of the univariate model,
and, with the exception of leaf lifespan (using mass-normalized traits), the multivariate model also outperformed the univariate model.
# TODO: Do a 20 or 30-fold cross validation to confirm this is not a fluke.
However, the differences between the grouped multivariate model and the hierarchical model were usually small,
and in some cases (e.g. mass- and area-normalized V_{c,max} and J_{max}),
the multivariate model actually slightly outperformed the hierarchical model.
...
# TODO: Re-run sample size analysis and create the corresponding figure
...
Theme: Leaf morphology and nutrient contents (SLA, N, P)
Results:
  - Within and across PFTs, direction of relationships was consistent, regardless of mass or area, but magnitude was variable, especially for mass-normalized traits.
  - Among tree PFTs, SLA with Nmass (and, to a lesser extent, Pmass) strongest in temperate, weaker in tropical and boreal.
  - Among non-tree PFTs, similar
  - Area patterns were more similar across PFT
Discussion:
  - The most robust feature of the leaf economic spectrum
  - Supported by many studies at different scales
  - But, even here, differences in slope magnitude reflect differences in allocation (Onoda, etc.)

Theme: Metabolism (Rd, Vcmax, Jmax)
Results:
  - Across PFT relationships are weaker than within-PFT
  - Within-PFT, generally consistent direction. Very stable Vcmax-Jmax slope, but magnitude of relationship of both traits with Rd more variable
  - Traits are usually positively correlated with SLA, N, and P, but with a few exceptions
    - Broadleaf deciduous temperate -- different direction for SLA-R_{d,area}, SLA-Jmax, and N-R_{d,area} (and P-R_{d,area})
    - Needleleaf evergreen temperate -- different direction for R_{d,area}-V_{c,max_area}
  - Lots more gaps due to missingness
Discussion:
  - Weaker across-PFT patterns indicative of the more dynamic nature of these traits.
  - Data limited for many PFTs -- need more pairwise measurements!
  - Consistent relationship of N_area with Vcmax supports the use of this relationships in ESMs
  - What's up with broadleaved deciduous temperate SLA/N/P-Rdarea, SLA/N/P-Jmaxarea?

Theme: Longevity (lifespan)
Results:
  - Most consistently strong set of across-PFT patterns, especially for mass-normalized traits
  - Most variable set of within-PFT patterns, especially for area-normalized traits
  - For mass-normalized traits, among deciduous PFTs, latitudinal gradient in LL-SLA and LL-N relationship -- more negative with increasing latitude
  - For area-normalized traits, patterns are generally weaker and more idiosyncratic
Discussion:
- Global patterns related to leaf longevity are a more robust feature of across- than within-PFT variability
- Fundamental difference between leaf longevity for deciduous and evergreen trees
  - Evergreen -- Driven by ???.
  - Deciduous -- Driven by seasonality. This also explains the latitudinal gradient among deciduous species.
#+END_COMMENT
...
# TODO: This is more nuanced -- discuss within- vs. across trends in leaf lifespan more here.
# TODO: Come back to this with a summary of the key results
Our results suggest that some dimensions of the leaf economic spectrum hold both within and across PFTs (at least at the functional and phylogenetic resolution of current Earth System Models),
while others are more nuanced.

In terms of direction, the most consistent relationships within- and across-PFTs were those among leaf morphology (SLA) and nutrient contents (N and P).
The directionality of relationships between these traits and metabolic traits (R_d, V_{c,max}, J_max) was more variable.
For example, temperate broadleaved deciduous trees were unique in showing positive relationships of SLA with R_{d,area} and J_{max,area} and negative relationships between N_area and R_{d,area}.
# TODO: ^^ Explain why
Meanwhile, temperate needleleaved evergreen trees were unique in showing a positive relationship between SLA and V_{c,max,area}, and a negative relationship between R_{d,area} and V_{c,max,area}.
# TODO: ^^ Explain why
Across PFT relationships for these traits were weak among mass-normalized traits, and even weaker among area-normalized traits.
The least consistent relationships in terms of direction were those related to leaf lifespan.
The across-PFT pattern followed the leaf economic spectrum, with leaf lifespan positively related to SLA and most mass-normalized traits.
However, the within-PFT relationships varied depending on leaf habit and latitude.
Namely, evergreen


had a distinct latitudinal gradient, with

Among mass-normalized traits, C3 arctic grasses and C4 grasses, stood out as having negative relationships between leaf lifespan and SLA, N, P, and R_d,
and temperate and boreal broadleaved deciduous trees similarly had

For C3 arctic grasses, to the extent that leaf lifespan can be treated as a proxy for temperature (which is reasonable given the strong direct and indirect controls of temperature on tundra phenology citealt:wipf_2009_winter),
this relationship can be interpreted as a climatic gradient, with plants from warmer areas (longer leaf lifespan) showing the expected preference for higher productivity (higher SLA and nutrient contents, and faster metabolism) over resilience cite:wright_2004_worldwide.


Both within and across PFTs, slopes of relationships between SLA, N_mass, and P_mass were consistently positive, though with varying magnitude.
Although we did not include maximum photosynthesis rate (A_max), closely related metabolic traits V_{c,max,mass} and J_{max,mass} generally exhibited the expected positive relationships with SLA and negative correlations with leaf lifespan, as did R_{d,mass},
though many PFTs had insufficient data to confidently estimate these relationships.




These same traits had negative relationships with leaf lifespan within all PFTs, with the notable exceptions of
temperate and boreal broadleaved deciduous trees (which showed positive SLA-leaf lifespan relationships) and
C3 arctic and C4 grasses (which had positive relationships between leaf lifespan and SLA, N_mass, P_mass, and R_{d,mass}).

# TODO: Probably drop this whole paragraph
...
For example, slopes of SLA with $N_{mass}$ and $P_{mass}$ were weaker in needleleaved PFTs compared to broadleaved PFTs.
Meanwhile, slopes of SLA with $N_{area}$ were strongly negative for all PFTs.
...
Relationships between leaf nutrient concentrations and traits related to photosynthetic metabolism ($V_{c,max}$ and $J_{max}$) are often used to parameterize photosynthesis in ecosystem models cite:clm45_note,rogers_2016_roadmap.
We found that, in general, these relationships are robust.
However, many PFTs--particularly those in high latitudes--lack the pairwise observations of metabolic and nutrient traits necessary to evaluate these relationships.

# TODO: This is not really true either, so probably drop a lot of it.
We found that the strength of these relationships was PFT-dependent.
...
# TODO: This is not really true. Revise this section.
