...
At the same time, the use of global among-trait and trait-environment correlations, for both ecological inference and predictive modeling, has several important caveats.
...
Similarly, let $x_i$ be the realization of the data $\mathbf{x}\prime$ with imputed values at MCMC iteration $i$.
...
A detailed demonstration of this multiple imputation approach and how it is used to estimate trait covariance is provided Supporting Information Method S1.
By performing imputation at every step of the algorithm, we integrate over the uncertainty in the missing data.
