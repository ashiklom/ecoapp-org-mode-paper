...
#+CAPTION: `uni` is the univariate model, by PFT;
#+CAPTION: `multi` is the multivariate model on all data ("global"), ignoring PFTs;
#+CAPTION: `multi_group` is the multivariate model fit separately to each PFT;
