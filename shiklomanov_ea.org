...
However, this may not be the case for the /next/ generation of ecosystem models, which may have different and possibly more detailed representations of plant functional diversity.
For example, such representations could consider systematic differences in trait values between regions with similar climates (e.g. tropical forests in South America and central Africa) revealed by spatially explicit trait analyses cite:butler_2017_mapping.
An emerging class of terrestrial ecosystem models with explicit representations of demographic processes (e.g. competition, ecological succession, mortality, recruitment) further disaggregate PFTs based on successional stage cite:longo_2019_ed1, or even eschew PFTs altogether in factor of modeling individual species cite:post_1996_linkages,weng_2015_scaling.
In both of these cases, the additional information provided by trait covariance (hierarchical or otherwise) could be valuable for overcoming the greater data limitation resulting from more finely-resolved plant groups.
Another approach is to allow PFTs to emerge from the data through classification and clustering techniques applied to functional traits and other ecological characteristics cite:boulangeat_2012_improving.
Trait covariance patterns such as those explored in our study could be used as inputs into such classification approaches in the future---for example, PFT definitions could be designed to maximize the differences in within- vs. across-PFT trait correlations.
This work would be further enhanced if combined with additional information from phylogenetic or taxonomic similarity cite:symonds_2014_primer.
Besides its utility to ecosystem modeling, such work would also contribute to outstanding questions in plant trait ecology about the extent to which leaf economic relationships hold within vs. across taxonomic levels (species, genus, family, clade), successional stages, or spatial domains.

# More generally, in all of these cases, the additional information provided by trait covariance, hierarchical or otherwise, would be useful for

# However, due to the combination of novel observations of plant functional diversity, more sophisticated simulations enabled by faster computers, and improved ecological understanding,

# modeling studies are increasingly breaking from the "traditional" PFT definitions used in this study, and the benefits of hierarchical modeling may be greater under these alternative definitions.
# For example, cite:boulangeat_2012_improving used an iterative classification approach based on species ecological characteristics to derive more detailed PFTs that better captured the diversity of their study area (Ecrins National Park, France).
