...
An alternative to using a single global set of trait trade-offs is to allow trait correlations to vary among PFTs (potentially with finer resolution, e.g. cite:boulangeat_2012_improving)
...
where $n$ is the number of observations and $m$ is the number of traits in data matrix $x$. For further details, see citet:gelman_2003_bayesian.
...
Moreover, accounting for covariance occasionally changed the /position/ of trait mean estimates, even for well-sampled PFT-trait combinations (e.g. $N_{mass}$ for temperate broadleaved deciduous trees, Fig. 4).
