...
#+LATEX_HEADER: \author[13]{Enio Egon Sosinski <sosinski@gmail.com>} %%% Emailed %%%
...
| Materials and methods |       1686 | 1935 | References | ??? |
| Results               |       1094 |      |            |     |
| Discussion            |       1524 |      |            |     |
| Conclusions           |        132 |      |            |     |
|-----------------------+------------+------+------------+-----|
| Total                 |       5570 | 5000 |            |     |
#+TBLFM: @4$2=$+1 - 249::@8$2=vsum(@I..II)
...
Trait data for this analysis comes from the TRY global traits database cite:kattge_2011_try (Tab.\ =\@ref(tab:trydata)=).
...
We restricted our analysis to TRY data that have been quality-controlled and for which adequate species information was provided for functional type classification [[cite:kattge_2011_try][see]].

Although maximum photosynthetic rate ($A_{max}$) was an important trait in previous studies, we did not include it for two reasons.
...
Where species attributes were missing, we assigned attributes based on higher order phylogeny if this was reasonably unambiguous (e.g., /Poaceae/ family are grasses, /Larix/ genus are deciduous needleleaved trees) and omitted the species if otherwise.
For biome specification, we collected all latitude and longitude data for each species,
matched these data to annual mean temperature ($AMT$, averaged 1970-2000) data from WorldClim-2 cite:fick_2017_worldclim,
...
The most complex model was the "hierarchical" model, where traits are drawn from a PFT-specific multivariate distribution describing within-PFT variation, and whose mean vector is sampled from a global multivariate distribution describing variation across PFTs.
...
Therefore, our algorithm included multiple imputation of partially missing data cite:white_2010_multiple,graham_2009_missing_data_analysis, which proceeded as follows:
...
Our results show that accounting for trait covariance reduced uncertainty around PFT-level trait means, particularly for trait-PFT combinations with low sample sizes (Fig.\ 2 and 3).
Moreover, accounting for trait covariance occasionally changed the /position/ of trait mean estimates, even for PFT-trait combinations with large sample sizes (e.g. $N_{mass}$ for temperate broadleaved deciduous trees, Fig.\ 2).
...
Such shifts suggest that the sampling of these traits in the TRY database is not representative (Fig.\ 1; see also citealt:kattge_2011_try). 
...
our results show that the effective sample size of all traits is enhanced when covariance is taken into account.
This means that field and remote sensing studies that estimate only certain traits (like SLA and $N_{mass}$)  may be able to leverage trait correlations to provide constraint on traits they do not directly observe (such as $P_{mass}$ and $R_{d,mass}$) cite:singh_2015_imaging,musavi_2015_imprint,lepine_2016_examining,serbin_2014_spectroscopic.
...
However, for modeling larger numbers of PFTs cite:boulangeat_2012_improving or even individual species [[cite:post_1996_linkages][e.g. Linkages, ]], the benefits of hierarchical modeling may accumulate cite:dietze_2008_capturing,cressie_2009_accounting,webb_2010_structured,clark_2004_why.
